...
export function runH4MobileCompaction(rootArg = process.cwd()) {
  ...
  const decision = readJson(resolve(root, '.stealtheye/state/fresh-tab-decision-core.json'), null as any) as any;
  const missingDecision = isEmpty(decision);

  const summary = {
    phase: 'H4',
    status: 'ACTIVE',
    next_action: missingDecision ? 'run h4:decision-core' : (decision?.next_action ?? 'run h4:decision-core'),
    escalation_status: missingDecision ? 'required' : (decision?.escalation_required ? 'required' : 'not-required'),
    approval_boundary_state: 'explicit-boundaries-preserved',
    branch_state: 'h4/autonomy-continuity-fresh-tab-runtime',
    compact: true,
    updated_at: now
  };

  writeFileSync(resolve(root, '.stealtheye/state/mobile-continuity-compaction.json'), JSON.stringify(summary, null, 2));
  writeFileSync(resolve(root, '.stealtheye/state/mobile-runtime-summary.json'), JSON.stringify({ continuity: missingDecision ? 'degraded' : 'bounded', replay: 'tracked', interruptions: 'recoverable', pr: 'continuity-guarded', blockers: missingDecision ? ['missing-decision-core-state'] : (decision?.blockers ?? []), updated_at: now }, null, 2));
