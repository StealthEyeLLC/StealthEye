...
export function runH4DecisionCore(rootArg = process.cwd()) {
  ...
  const continuity = readJson(resolve(root, '.stealtheye/state/continuity-fabric-state.json'), null as any) as any;
  ...
  if (isEmpty(continuity)) {
    blockers.push('missing-continuity-fabric-state');
  } else {
    blockers.push(...(continuity.continuity_blockers ?? []));
  }
  ...
  const core = {
    ...
    status: 'ACTIVE',
    next_action: action,
    why: blockers.length ? 'blockers-detected' : 'safe-autonomous-continuation',
    evidence: ['repo-native state precedence', 'continuity fabric', 'runtime matrix'],
    blockers,
    escalation_required: blockers.length > 0,
    human_action_needed: blockers.includes('invalid-escalation-path'),
    safe_to_continue: blockers.length === 0,
    continuity_confidence: continuity?.continuity_confidence ?? 'low',
    recommended_validation_chain: ['h4:validate','h4:governance-check','h4:determinism-check'],
    recommended_pr_action: 'continue h4 hardening PR only',
    recommended_runtime_action: 'bounded deterministic run',
    updated_at: now
  };
  ...
  writeFileSync(resolve(root, '.stealtheye/state/fresh-tab-decision-risks.json'), JSON.stringify({ risks: blockers, escalation_severity: blockers.length ? 'high' : 'low', safe_autonomy_level: blockers.length ? 'restricted-autonomy' : 'bounded-autonomous', updated_at: now }, null, 2));
