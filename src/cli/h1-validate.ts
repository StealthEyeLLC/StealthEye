...
const mem = readJson('.stealtheye/state/h1-browser-execution-memory.json', { runs: [] }) as any;
const readiness={status:latest.status==='ok'?'validated':'partial',execution_body_readiness:true,browser_readiness:posture.classes.includes('local-browser-available')||posture.classes.includes('github-actions'),routing_readiness:true,repair_loop_readiness:true,governance_readiness:true,runtime_readiness:!posture.classes.includes('browser-missing') || posture.classes.includes('github-actions'),environment_restrictions:posture.classes.filter(x=>x==='external-blocked'||x==='codex-restricted'),ci_proof_readiness:true,ci_proof_success:latest.status==='ok',replay_integrity:!!latest.replay_seed,proof_continuity:!!latest.proof_id,browser_operational_readiness:latest.operational_scores?.browser_operational_readiness ?? 0.6,replay_reproducibility_score:latest.stability_scores?.proof_reproducibility_score ?? 0,repair_reliability:Math.max(0,1-((mem.repair_outcomes||[]).slice(-10).filter((x:any)=>x.retryable===false).length/10))};
const gaps={status:'ok',remaining_blockers:readiness.ci_proof_success?[]:['ci-proof-not-yet-successful'],remaining_risks:readiness.environment_restrictions,routing_maturity:latest.routing?.decision ?? 'pending',browser_body_readiness:readiness.browser_operational_readiness,ci_authority_score:latest.operational_scores?.ci_proof_stability ?? 0,replay_reproducibility_score:readiness.replay_reproducibility_score,execution_autonomy_score:latest.operational_scores?.browser_autonomy_readiness ?? 0};
